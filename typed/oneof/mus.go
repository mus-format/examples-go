...
func (s instructionMUS) Marshal(in Instruction, bs []byte) (n int) {
	if m, ok := in.(mus.MarshallerTyped); ok {
		return m.MarshalTypedMUS(bs)
	}
	panic(fmt.Sprintf("%v doesn't implement the mus.MarshallerTypedMUS interface",
		reflect.TypeOf(in)))
}
...
func (s instructionMUS) Unmarshal(bs []byte) (in Instruction, n int, err error) {
	...
	var n1 int
	switch dtm {
	case CopyTypedMUS.DTM():
		in, n1, err = CopyTypedMUS.UnmarshalData(bs[n:])
		n += n1
	case InsertTypedMUS.DTM():
		in, n1, err = InsertTypedMUS.UnmarshalData(bs[n:])
		n += n1
	default:
...
func (s instructionMUS) Size(in Instruction) (size int) {
	if s, ok := in.(mus.MarshallerTyped); ok {
